...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define STM32_FAMILY_ID 0xF0                     /* For STM32F0 devices */
#define BOARD_ID        0x0                      /* BOARD_ID: from 0 to 7: Device number if two
                                                    boards using STM32F0, need to select another value (1 to 7) */
#define TX_ID  ((BOARD_ID<<8) | STM32_FAMILY_ID) /* TX ID to send is a format of 0xXF0, where X is the board ID */
/* USER CODE END PD */
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *CanHandle)
{
    ...
    if ((RxHeaderFIFO0.StdId & 0xFF) == 0xF1)
    {
